...
export class JobCandidateService {
  ...
  async assignCandidate (selectJobCandidate: SelectJobCandidateDto) {
    ...
    if (jobCandidate.selected)
      throw  new BadRequestException("This candidate was already assigned to the requested Job");

    if (jobCandidate.job.status != JobStatus.PUBLISHED)
      throw  new BadRequestException("Incorrect Job Status: ["
        + jobCandidate.job.status + "], Expected [published]");

    //Update JOB
    const updatedJob = await this.jobsService.assignJob(jobCandidate.job, jobCandidate.driver)
    ...
    if (updatedJob.status == JobStatus.ASSIGNED) {
      ...
      throw new InternalServerErrorException("Candidate was not assigned to the requested Job.")
    }
  }
}
